...
- **Rotation** : **décision retenue — 90 jours, alignée sur la cadence du jeton API GitLab** (voir [Sécurisation de l'appel sortant vers l'API GitLab](detection-defauts.md#securisation-de-lappel-sortant-vers-lapi-gitlab)) — non pas parce que ce secret expirerait de lui-même (GitLab ne lui impose aucune expiration, contrairement au jeton API), mais pour ne tenir qu'un seul calendrier de rotation pour les deux secrets de ce service plutôt que deux cadences différentes sans justification technique propre. La conséquence d'un retard diffère aussi du jeton API : ici, rien ne casse automatiquement — le secret reste valide tant qu'il n'est pas changé des deux côtés (GitLab et `SYNC_WEBHOOK_SECRET`). Le risque d'un retard est donc une fenêtre d'exposition prolongée en cas de fuite non détectée — un risque de sécurité, pas de disponibilité — ce qui ne justifie pas la même surveillance automatisée par seuil : `ROTATED_AT`, déjà présent dans la table, suffit à documenter pour un auditeur la conformité de cette rotation manuelle, sans nouveau job TWS/OPC dédié à construire.
