...
Dans la même lignée, un autre outil est destiné à archiver les sources obsolètes ainsi que les artefacts de production correspondants, afin de garantir strictement la bijection imposée par l'IG même après le retrait d'un composant du périmètre actif.

Cet archivage ne se limite pas à la paire « source COBOL + load » qui sert de fil rouge à cette page. Ce que l'outil doit retrouver, pour chaque composant retiré, c'est **ce qui a réellement tourné en production** et le commit GitLab dont cela provient — et cette correspondance ne prend pas la même forme selon la technologie :

- pour un programme compilé (COBOL/C/HLASM, ou un artefact Java/Python/Node.js), c'est la paire source/load ou source/manifeste déjà décrite dans [Prise d'image du patrimoine](#prise-dimage-du-patrimoine-en-production) ;
- pour une **procédure stockée SQL native**, il n'y a pas de load : le lien s'établit entre le source SQL PL versionné sur GitLab/USS et le package correspondant du catalogue DB2 de production (`SYSIBM.SYSPACKAGE`), identifié par sa clause `VERSION` (voir plus haut, note sur les procédures stockées SQL natives) ;
- pour un **script shell (ou REXX)**, il n'y a ni load ni manifeste : le script exécuté en production est, littéralement, le même texte que la source. La difficulté n'est donc pas de relier deux artefacts distincts, mais de prouver **quelle version exacte** était en place à un instant donné en production — en l'absence d'IDR ou de manifeste, cette preuve s'appuierait plutôt sur les métadonnées de déploiement déjà évoquées pour le tatouage, en particulier le tag de déploiement posé sur le commit (voir [Rétention et purge des objets git](architecture/resilience/service-synchronisation.md#retention-et-purge-des-objets-git)).

Cette question — établir la bijection pour chaque type de composant déployé, au-delà du seul couple source/load compilé — reste entière et relève entièrement de l'outil d'archivage et du projet qui le porte, un projet connexe distinct (voir l'avertissement en tête de cette page). Ce miroir USS n'y contribue qu'en amont, en garantissant que le source archivé sur USS reste fidèle à GitLab au moment du retrait.

Dans ce cadre, le miroir USS serait actualisé par le mécanisme de synchronisation dès qu'une suppression logique est effectuée sur GitLab. L'archivage consiste à retirer les sources et les artefacts de production correspondants du périmètre actif tout en maintenant leur accessibilité via des processus dédiés — une démarche cohérente avec les obligations générales de conservation et de traçabilité documentaire applicables au secteur bancaire, sans qu'un texte impose littéralement l'archivage de ce couple source/artefact.

Pour les composants modernes (Java, CICS-OSGi, Python, Node.js), c'est l'**identifiant de package lu dans le manifeste** de l'artefact qui permet à cet outil de retrouver le projet source correspondant — l'unité archivée est donc le **projet pointé par le manifeste**, pas un fichier source isolé, cohérent avec le grain de la bijection retenu pour ces écosystèmes.
